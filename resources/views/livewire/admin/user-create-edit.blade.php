<x-admin.form-section submit="saveOrUpdate">
    <x-slot name="form">
                    ...
                    <x-admin.form-group>
                        <x-admin.lable value="Industry" />
                        <x-admin.dropdown  wire:model.defer="industry" placeHolderText="Please select one" autocomplete="off" class="{{ $errors->has('industry') ? 'is-invalid' :'' }}">
                                @foreach ($industryList as $status)
                                    <x-admin.dropdown-item  :value="$status['value']" :text="$status['text']"/>                          
                                @endforeach
                        </x-admin.dropdown>
                        <x-admin.input-error for="industry" />
                    </x-admin.form-group>
                    <x-admin.form-group>
                        <x-admin.lable value="Activity" />
                        <x-admin.dropdown  wire:model.defer="activity" placeHolderText="Please select one" autocomplete="off" class="{{ $errors->has('activity') ? 'is-invalid' :'' }}">
                                @foreach ($activityList as $status)
                                    <x-admin.dropdown-item  :value="$status['value']" :text="$status['text']"/>                          
                                @endforeach
                        </x-admin.dropdown>
                        <x-admin.input-error for="activity" />
                    </x-admin.form-group>
                    ...
                    <x-admin.lable value="Details" />
                    <textarea
                    x-data x-init="editor = CKEDITOR.replace('details');
                    editor.on('change', function(event){
                        @this.set('details', event.editor.getData());
                    })" wire:model.defer="details" id="details" class="form-control {{ $errors->has('details') ? 'is-invalid' :'' }}"></textarea>
                    </x-admin.form-group>
