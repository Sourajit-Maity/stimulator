    ...
    <x-slot name="thead">
        <tr role="row">
            <th tabindex="0" aria-controls="kt_table_1" rowspan="1" colspan="1" style="width: 22%;"
                aria-sort="ascending" aria-label="Agent: activate to sort column descending">Bcd Percentage <i
                    class="fa fa-fw fa-sort pull-right" style="cursor: pointer;" wire:click="sortBy('bcd_percentage')"></i>
            </th>
            <th tabindex="0" aria-controls="kt_table_1" rowspan="1" colspan="1" style="width: 23%;"
                aria-label="Company Email: activate to sort column ascending">Bcd Actual <i
                    class="fa fa-fw fa-sort pull-right" style="cursor: pointer;" wire:click="sortBy('bcd_actual')"></i></th>
    ...
    <x-slot name="tbody">
        @forelse($datas as $data)
            <tr role="row" class="odd">   
                <td>{{ $data->bcd_percentage }}</td>
                <td>{{ $data->bcd_actual }}</td>
                ...
                <x-admin.td-action>
                    <a class="dropdown-item" href="{{ route('bcd.edit', ['bcd' => $data->id]) }}"><i
