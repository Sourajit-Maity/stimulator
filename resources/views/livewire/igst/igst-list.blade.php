    ...
    <x-slot name="thead">
        <tr role="row">
            <th tabindex="0" aria-controls="kt_table_1" rowspan="1" colspan="1" style="width: 22%;"
                aria-sort="ascending" aria-label="Agent: activate to sort column descending">Igst Percentage <i
                    class="fa fa-fw fa-sort pull-right" style="cursor: pointer;" wire:click="sortBy('igst_percentage')"></i>
            </th>
            <th tabindex="0" aria-controls="kt_table_1" rowspan="1" colspan="1" style="width: 23%;"
                aria-label="Company Email: activate to sort column ascending">Igst Actual <i
                    class="fa fa-fw fa-sort pull-right" style="cursor: pointer;" wire:click="sortBy('igst_actual')"></i></th>
    ...
    <x-slot name="tbody">
        @forelse($datas as $data)
            <tr role="row" class="odd">   
                <td>{{ $data->igst_percentage }}</td>
                <td>{{ $data->igst_actual }}</td>
                ...
                <x-admin.td-action>
                    <a class="dropdown-item" href="{{ route('igst.edit', ['igst' => $data->id]) }}"><i
