    ...
    <x-slot name="thead">
        <tr role="row">
            <th tabindex="0" aria-controls="kt_table_1" rowspan="1" colspan="1" style="width: 22%;"
                aria-sort="ascending" aria-label="Agent: activate to sort column descending">Rodtep Percentage <i
                    class="fa fa-fw fa-sort pull-right" style="cursor: pointer;" wire:click="sortBy('rodtep_percentage')"></i>
            </th>
            <th tabindex="0" aria-controls="kt_table_1" rowspan="1" colspan="1" style="width: 23%;"
                aria-label="Company Email: activate to sort column ascending">Rodtep Actual <i
                    class="fa fa-fw fa-sort pull-right" style="cursor: pointer;" wire:click="sortBy('rodtep_actual')"></i></th>
    ...
    <x-slot name="tbody">
        @forelse($datas as $data)
            <tr role="row" class="odd">   
                <td>{{ $data->rodtep_percentage }}</td>
                <td>{{ $data->rodtep_actual }}</td>
                ...
                <x-admin.td-action>
                    <a class="dropdown-item" href="{{ route('rodtep.edit', ['rodtep' => $data->id]) }}"><i
