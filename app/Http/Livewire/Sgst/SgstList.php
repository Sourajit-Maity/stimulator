...
class SgstList extends Component
{
    use WithPagination;
    use WithSorting;
    use AlertMessage;
    ...
    {
        $dataQuery = Sgst::query();
        if ($this->searchPercentage)
            $dataQuery->orWhere('sgst_percentage', 'like', '%' . $this->searchPercentage . '%');

        if ($this->searchNumber)
            $dataQuery->orWhere('sgst_actual', 'like', '%' . $this->searchNumber . '%');
        if ($this->searchStatus >= 0)
            $userQuery->orWhere('active', $this->searchStatus);
        return view('livewire.sgst.sgst-list', [
            'datas' => $dataQuery
    ...
    {
        Sgst::destroy($id);
        $this->showModal('success', 'Success', 'Sgst  has been deleted successfully');
    }
    ...
    public function changeStatus(Sgst $data)
    {
        $data->fill(['active' => ($data->active == 1) ? 0 : 1])->save();

        $this->showModal('success', 'Success', 'Sgst status has been changed successfully');
    }
}
