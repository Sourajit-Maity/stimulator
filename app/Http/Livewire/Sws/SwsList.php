...
class SwsList extends Component
{
    use WithPagination;
    use WithSorting;
    use AlertMessage;
    ...
    public function render()
    {
        ...
        if ($this->searchPercentage)
            $dataQuery->orWhere('sws_percentage', 'like', '%' . $this->searchPercentage . '%');

        if ($this->searchNumber)
            $dataQuery->orWhere('sws_actual', 'like', '%' . $this->searchNumber . '%');
        if ($this->searchStatus >= 0)
            $userQuery->orWhere('active', $this->searchStatus);
        return view('livewire.sws.sws-list', [
            'datas' => $dataQuery
    ...
    {
        Sws::destroy($id);
        $this->showModal('success', 'Success', 'Sws  has been deleted successfully');
    }
    ...
    public function changeStatus(Sws $data)
    {
        $data->fill(['active' => ($data->active == 1) ? 0 : 1])->save();

        $this->showModal('success', 'Success', 'Sws status has been changed successfully');
    }
}
