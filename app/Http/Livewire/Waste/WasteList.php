...
class WasteList extends Component
{
    ...
    {
        $dataQuery = WasteScrap::query();
        if ($this->searchPercentage)
            $dataQuery->orWhere('waste_scrap_percentage', 'like', '%' . $this->searchPercentage . '%');

        if ($this->searchNumber)
            $dataQuery->orWhere('waste_scrap_actual', 'like', '%' . $this->searchNumber . '%');
        if ($this->searchStatus >= 0)
            $userQuery->orWhere('active', $this->searchStatus);
        return view('livewire.waste.waste-list', [
            'datas' => $dataQuery
    ...
    {
        WasteScrap::destroy($id);
        $this->showModal('success', 'Success', 'WasteScrap  has been deleted successfully');
    }
    ...
    public function changeStatus(WasteScrap $data)
    {
        $data->fill(['active' => ($data->active == 1) ? 0 : 1])->save();

        $this->showModal('success', 'Success', 'WasteScrap status has been changed successfully');
    }
}
