...
class IgstList extends Component
{
    use WithPagination;
    use WithSorting;
    use AlertMessage;
    ...
    {
        $dataQuery = Igst::query();
        if ($this->searchPercentage)
            $dataQuery->orWhere('igst_percentage', 'like', '%' . $this->searchPercentage . '%');

        if ($this->searchNumber)
            $dataQuery->orWhere('igst_actual', 'like', '%' . $this->searchNumber . '%');
        if ($this->searchStatus >= 0)
            $userQuery->orWhere('active', $this->searchStatus);
        return view('livewire.igst.igst-list', [
            'datas' => $dataQuery
    ...
    {
        Igst::destroy($id);
        $this->showModal('success', 'Success', 'Igst  has been deleted successfully');
    }
    ...
    public function changeStatus(Igst $data)
    {
        $data->fill(['active' => ($data->active == 1) ? 0 : 1])->save();

        $this->showModal('success', 'Success', 'Igst status has been changed successfully');
    }
}
