...
class AirList extends Component
{
    ...
    public function render()
    {
        ...
        if ($this->searchPercentage)
            $dataQuery->orWhere('air_percentage', 'like', '%' . $this->searchPercentage . '%');

        if ($this->searchNumber)
            $dataQuery->orWhere('air_actual', 'like', '%' . $this->searchNumber . '%');
        if ($this->searchStatus >= 0)
            $userQuery->orWhere('active', $this->searchStatus);
        return view('livewire.air.air-list', [
            'datas' => $dataQuery
    ...
    public function changeStatus(Air $data)
    {
