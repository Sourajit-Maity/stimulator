...
class RodtepList extends Component
{
    ...
    {
        $dataQuery = Rodtep::query();
        if ($this->searchPercentage)
            $dataQuery->orWhere('rodtep_percentage', 'like', '%' . $this->searchPercentage . '%');

        if ($this->searchNumber)
            $dataQuery->orWhere('rodtep_actual', 'like', '%' . $this->searchNumber . '%');
        if ($this->searchStatus >= 0)
            $userQuery->orWhere('active', $this->searchStatus);
        return view('livewire.rodtep.rodtep-list', [
            'datas' => $dataQuery
    ...
    {
        Rodtep::destroy($id);
        $this->showModal('success', 'Success', 'Rodtep  has been deleted successfully');
    }
    ...
    public function changeStatus(Rodtep $data)
    {
        $data->fill(['active' => ($data->active == 1) ? 0 : 1])->save();

        $this->showModal('success', 'Success', 'Rodtep status has been changed successfully');
    }
}
